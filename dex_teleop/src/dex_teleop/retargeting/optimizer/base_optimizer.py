...
# Retargeting package root for URDF path resolution.
...
class BaseOptimizer(ABC):
    ...
    def __init__(self, config: dict):
        ...
        if robot_type not in self.ROBOT_CONFIGS:
            raise ValueError(f"Unsupported robot.type {robot_type!r}; choose from {tuple(self.ROBOT_CONFIGS)}")
        ...
        # Handle left/right hand prefixes.
        # The config uses 'rh_' prefix by default, replace with 'lh_' for left hand
        if robot_type == 'shadow' and self.hand_side == 'left':
        ...
        elif robot_type in {'sharpa', 'wuji'} and self.hand_side == 'left':
            def replace_prefix(name):
