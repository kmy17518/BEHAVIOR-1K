...
def _validate_object_state_stability(obj_name, obj_dict, strict=False):
    ...
    for key, atol in zip(("lin_vel", "ang_vel"), (lin_vel_threshold, ang_vel_threshold)):
        ...
        if not th.all(th.isclose(th.as_tensor(val), th.zeros(1), atol=atol, rtol=0.0)).item():
    ...
    if "joint_vel" in obj_dict.keys():
        ...
        if not th.all(th.isclose(th.as_tensor(val), th.zeros(1), atol=joint_vel_threshold, rtol=0.0)).item():
...
def validate_task(task, task_scene_dict, default_scene_dict):
    ...
    def _validate_identical_object_kinematic_state(obj_name, default_obj_dict, obj_dict, check_vel=True):
        ...
        for key, val in default_obj_dict["root_link"].items():
            ...
            if not check_vel and "vel" in key:
                continue
            # Skip if key doesn't exist in obj_dict (e.g., lin_vel/ang_vel may not be present)
            ...
            if "particle" in key:
                ...
                if key == "ori":
                    ...
                    # Convert to tensors to avoid torch dynamo issues with list inputs
                    val_tensor = th.as_tensor(val)
                    obj_val_tensor = th.as_tensor(obj_val)
                    obj_val = th.norm(T.quat2axisangle(T.quat_distance(val_tensor, obj_val_tensor)))
                    val = 0.0
                if not th.all(th.isclose(th.as_tensor(val), th.as_tensor(obj_val), atol=atol, rtol=0.0)).item():
                    return (
                        False,
    ...
    for obj_name, default_obj_info in default_scene_dict["state"]["registry"]["object_registry"].items():
        ...
        if not valid_obj:
            print(f"Failed validation step 2: stable scene state and task scene state do not have similar kinematic states for non-task-relevant objects. Specific error: {err_msg}")
            # return (
            #     False,
