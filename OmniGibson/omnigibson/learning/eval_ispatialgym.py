...
TASK_NAMES_TO_INDICES = {
    "camera_relocalization": 0,
    "path_integration": 1,
}

m = create_module_macros(module_path=__file__)
m.NUM_EVAL_EPISODES = 1
m.NUM_EVAL_INSTANCES = 1
...
class Evaluator:
    ...
    def load_env(self, env_wrapper: DictConfig) -> EnvironmentWrapper:
        ...
        with open(os.path.join(gm.DATA_PATH, "ispatialgym-instances", "metadata", "episodes.jsonl"), "r") as f:
        ...
        cfg["task"]["termination_config"]["max_steps"] = 500 # TODO: Temporary fix
    ...
    def step(self) -> Tuple[bool, bool]:
        ...
        """
        # TODO: Temporarily commented out websocket policy - using random actions instead
        # self.robot_action = self.policy.forward(obs=self.obs)
        self.robot_action = self.env.action_space.sample()
    ...
    def load_task_instance(self, instance_id: int) -> None:
        ...
        for tro_key, tro_value in tro_state.items():
            if tro_key == "robot_poses":
                ...
                self.robot.pose_goal = tro_value
...
if __name__ == "__main__":
    ...
    with hydra.initialize_config_dir(f"{Path(getsourcefile(lambda:0)).parents[0]}/configs", version_base="1.1"):
        config = hydra.compose("ispatialgym_config.yaml", overrides=sys.argv[1:])
    ...
    task_instance_csv_path = os.path.join(
        gm.DATA_PATH, "ispatialgym-instances", "metadata", "test_instances.csv"
    )
    ...
    logger.info(f"Instances to run: {instances_to_run}")
    # establish metrics
    metrics = {}
